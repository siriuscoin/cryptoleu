...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("siriuscoin-ext-ip");
...
void SocketStoreData(CNode *pnode)
{
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Siriuscoin " + FormatFullVersion();
...
//	{"siriuscoin.org", "seed.siriuscoin.org"},
...
//	{"siriuscoin.org", "testnet-seed.siriuscoin.org"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Siriuscoin is probably already running."), addrBind.ToString().c_str());
        else
...
{
    printf("StopNode()\n");
    GenerateSiriuscoins(false, NULL);
    MapPort(false);
