        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-cryptoleu-address
...
string CWallet::StoreData(CWalletTx& wtxNew, bool fAskFee, const std::string& txData)
{
    // Check amount
    ...
    printf("StoreData(): nFeeRequired = %d\n", nFeeRequired);
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CSiriuscoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CSiriuscoinAddress(address).ToString());
}
