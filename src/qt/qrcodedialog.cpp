...
{
    QString ret = QString("siriuscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CLC unit input in lnReqAmount, we generate the URI with CLC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SiriuscoinUnits::format(SiriuscoinUnits::CLC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
