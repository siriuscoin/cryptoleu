...
#ifndef SIRIUSCOIN_MAIN_H
#define SIRIUSCOIN_MAIN_H
...
static const unsigned int MAX_TX_DATA_SIZE = (512 * 1024);
...
static const int64 MAX_MONEY = 20121641 * COIN;
...
public:
    ...
    */
    ...
    /** Amount of siriuscoins spent by this transaction.
    ...
    /** Amount of siriuscoins coming in to this transaction
 ...
 *               * 8358: compact amount representation for 60000000000 (600 CLC)
 ...
 *             * 86ef97d579: compact amount representation for 234925952 (2.35 CLC)
 *             * 00: special txout type pay-to-pubkey-hash
 *             * 61b01caab50f1b8e9c50a5057eb43c2d9563a4ee: address uint160
 *  - vout[16]: bbd123008c988f1a4a4de2161e0f50aac7f17e7f9555caa4
 *              * bbd123: compact amount representation for 110397 (0.001 CLC)
 *              * 00: special txout type pay-to-pubkey-hash
...
public:
    ...
    uint256 GetHash() const
    {
        return HashSHA3(BEGIN(nVersion), END(nNonce));
    }
...
public:
    ...
    uint256 BuildMerkleTree() const
    {
        ...
        for (int nSize = vtx.size(); nSize > 1; nSize = (nSize + 1) / 2)
        {
            for (int i = 0; i < nSize; i += 2)
            {
                ...
                vMerkleTree.push_back(HashSHA3(BEGIN(vMerkleTree[j+i]),  END(vMerkleTree[j+i]),
    ...
    static uint256 CheckMerkleBranch(uint256 hash, const std::vector<uint256>& vMerkleBranch, int nIndex)
    {
        ...
        {
            if (nIndex & 1)
                hash = HashSHA3(BEGIN(otherside), END(otherside), BEGIN(hash), END(hash));
            else
                hash = HashSHA3(BEGIN(hash), END(hash), BEGIN(otherside), END(otherside));
            nIndex >>= 1;
        }
        return hash;
    }
