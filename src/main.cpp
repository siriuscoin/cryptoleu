...
const string strMessageMagic = "Siriuscoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uCLC
...
static const int64 nTargetTimespan = 1000; // One thousand second
static const int64 nTargetSpacing = 10; // Ten second
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // Re-target every one hundred blocks
...
void ThreadScriptCheck() {
    RenameThread("siriuscoin-scriptch");
...
uint256 CPartialMerkleTree::CalcHash(int height, unsigned int pos, const std::vector<uint256> &vTxid) {
    if (height == 0) {
        ...
        if (pos*2+1 < CalcTreeWidth(height-1))
            ...
            right = left;
        // combine subhashes
        return HashSHA3(BEGIN(left), END(left), BEGIN(right), END(right));
    }
}
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 01/Jan/2014 Veteran comedian John Fortune dies, aged 74";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d5f2b1372fcad0100373f8a9d34fc43e80dcd56275c8fdaa64766f220cc88c8bda4288f9d7eb91273c2a6a75f165212624e139267cf66b824e6ad53ce2102eef") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1389052800;
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = HashSHA3(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = HashSHA3(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = HashSHA3(vRecv.begin(), vRecv.begin() + nMessageSize);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SiriuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SiriuscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SiriuscoinMiner(CWallet *pwallet)
{
    printf("SiriuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("siriuscoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running SiriuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//				nBlockBits = ByteReverse(pblock->nBits);
    ...
    {
        printf("SiriuscoinMiner terminated\n");
        throw;
    }
}

void GenerateSiriuscoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SiriuscoinMiner, pwallet));
}
